...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
		...
		int curTid = myTcb.getTid(); // get the current thread id

		/* let the current thread sleep in waitQueue under the
		condition = this thread id */
		return waitQueue.enqueueAndSleep(curTid); // return a child thread id who woke me up
		case EXIT:
		
		myTcb = scheduler.getMyTcb();
		int parId = myTcb.getPid(); // get the current thread's parent id
		curTid = myTcb.getTid(); // get the current thread id
		
		/* search waitQueue for and wakes up the thread under the 
		condition = the current thread's parent id */
		waitQueue.dequeueAndWakeup(parId, curTid);
